...
class InstitutionListView(View):
    ...
    async def get(self, request):
        ...
        fetched = await fetch_data(endpoint)
        all_institutions = fetched.get("institutions", [])
...
class DetailInstitutionView(View):
    ...
    async def get(self, request, cnpj):
        endpoint = settings.GET_INSTITUTION.format(cnpj=cnpj)
        institution = await fetch_data(endpoint)
...
class FilterInstitutionView(View):
    ...
    async def get(self, request, filter_by, value):
        if filter_by == "state":
