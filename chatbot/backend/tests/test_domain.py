...
def test_registration_success():
    ...
    flow = RegistrationFlow(
        repo, MockAddressProvider(), MockImageModerationService(), MockSpeechService()
    )

    request = RegistrationRequest(
        cnpj="12345678901234", name="ONG Teste", email="contato@ong.com"
    )
    ...
    assert (
        repo.created_data["confirmation_audio"]
        == "https://mock-s3-bucket.s3.amazonaws.com/mock_audio.mp3"
    )
...
def test_list_by_region():
    ...
    request = ListInstitutionRequest(filter_type="região", region="Sudeste")
