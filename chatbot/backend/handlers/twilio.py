...
def twilio_webhook(event, context):
    ...
    if queue_url:
        sqs_client.send_message(QueueUrl=queue_url, MessageBody=json.dumps(params))
...
@idempotent_function(
    data_keyword_argument="message_sid", persistence_store=persistence_layer
)
...
def twilio_worker(event, context):
    ...
    for record in sqs_event.records:
        try:
            ...
            process_single_message(message_sid=message_sid, params=params)
